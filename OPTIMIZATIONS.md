...
| 20 | The fused SDPA set is discrete, not a range | **KEPT** (a fact, not a change) | `profiling/probes/sdpa_dispatch.py` | `head_dim` 1..288, every mask kind, every dtype, `S` 512 and 1024, `B*H` 1 to 256 | the set is {64, 72, 80, 96, 128}. 65, 100 and 127 fall back. Always pad to the SMALLEST member at or above `head_dim`: a larger target lost all 44 rows |
...
| 44 | Chain `ffn_in` and `ffn_out` into one kernel | **REVERTED** | `profiling/probes/chain_probe.py`, not in the model | — | built, correct, and it LOSES. Best of 40 configurations is **0.969x**, and a repeat run gave 0.960x. The chain deletes the 128 MiB `hidden` round trip, worth about 1.1 ms, but one threadgroup must own all of `ffn_dim` to hold the row, and the threadgroup memory that costs takes more than the round trip gives. Controlled pair at `bm = 32`: `bk8` uses 24.0 KiB and reaches 0.996x, `bk16` uses 29.0 KiB and reaches 0.896x — same tile, more threadgroup, worse time. Relative error 3e-07, so it is correct, just slower |
...
in `profiling/results/backend_comparison.json`, 300 samples for each backend:
...
`appendix_cases.py` disables shape 14 (B=32, D=1024, H=16, S=100000, L=2),
and it must keep it disabled. That file compares against
`BaselineTransformer`, which cannot run the shape: `BaselineSelfAttention`
builds a B x H x S x S score matrix, which is 18.6 TiB.

The MLX path CAN run shape 14. `shape14_harness.py` runs it, times it, and
checks it without a full-shape baseline. See row 55.
...
gave a useless comparison. That reading stays in `profiling/results/history.jsonl`
...
Found by `profiling/probes/stage_roofline.py`. The two LayerNorm calls of one block
...
Found by `profiling/probes/stage_roofline.py`. An MLX transpose is a **free strided
view**, not a copy:
...
Found by `profiling/probes/stage_roofline.py`. Every other stage of the shape 6
...
The CPU column came from `profiling/results/cpu_cache.json` on this sweep, so the
...
A stage roofline run on all 13 shapes placed this row. Command:
    ...
    .venv/bin/python3 profiling/probes/stage_roofline.py --shapes 6,8
    .venv/bin/python3 profiling/probes/stage_roofline.py --shapes 1,2,3,4,5,7,9,10,11,12,13
    .venv/bin/python3 profiling/probes/stage_roofline.py --shapes 13
...
`profiling/probes/chain_probe.py`:
...
`profiling/probes/tile_probe.py` measured the `bn = ffn_dim` tile at 0.884x before
...
is 1.27x, and `profiling/probes/ln_absorb_probe.py` predicted 1.0x to 1.2x before
...
`profiling/probes/ln_tiled_stats_probe.py`, which writes no Metal.
...
real forward:
    ...
    .venv/bin/python3 profiling/probes/ln_tiled_stats_probe.py --shape 6
    .venv/bin/python3 profiling/probes/ln_tiled_stats_probe.py --shape 8
    .venv/bin/python3 profiling/probes/ln_tiled_stats_probe.py --shape 13
...
`profiling/probes/stage_roofline.py` still times `ln1 stats` and `ln2 stats` as
...
`profiling/probes/pipeline_probe.py` holds it, with six arms. Every arm is bit
...
`mx.eval` and `mx.synchronize`:
    ...
    .venv/bin/python3 profiling/probes/d_outer_probe.py --repeats 40 --rounds 3
...
width (M = 131072, d_model = 128):
    ...
    .venv/bin/python3 profiling/probes/final_ln_probe.py
...
`profiling/probes/ln_tiled_stats_probe.py` measures that this model stays far below
...
`profiling/probes/final_ln_probe.py`, interleaved, median of 60:
...
`final_ln`, alternating the order each round:
    ...
    .venv/bin/python3 profiling/probes/plan_ab.py --cases 6 --repeats 8 --rounds 5
    .venv/bin/python3 profiling/probes/plan_ab.py --cases 13,8,5,1,12 --repeats 30 --rounds 5
    .venv/bin/python3 profiling/probes/plan_ab.py --cases 2,3,7,9,10,11,4 --repeats 60 --rounds 5
...
on separate invocations:
    ...
    .venv/bin/python3 profiling/probes/plan_ab.py --cases 6,8,13 --repeats 8 --rounds 7
    .venv/bin/python3 profiling/probes/plan_ab.py --cases 1,2,3,4,5,7,9,10,11,12 --repeats 60 --rounds 7
...
`profiling/tools/gpu_timeline.py` records a `Metal System Trace` and reads it with
`xctrace export`, so no Xcode window is needed.

    ./profiling/tools/gpu_timeline.sh --case 6 --iterations 5 --warmup 3
...
## 55. Run appendix shape 14 through a separate harness — KEPT

Shape 14 is `B=32, D=1024, H=16, S=100000, L=2`, causal, `ffn_dim=1024`.
Every earlier version of this log called it impossible. That ran two
separate facts together, and only one of them is true.

**The baseline cannot run it, and no machine can.** `BaselineSelfAttention`
materializes a `B x H x S x S` score matrix, which is 18.6 TiB. It is 596
GiB for a single batch row of a single layer. The torch baseline therefore
stops with an allocation error before it computes anything, on the CPU and
on MPS alike. It has no run time at this shape.

**The MLX path CAN run it.** `head_dim = 1024 / 16 = 64`, which is in the
fused set of rows 18 and 20. So `mx.fast.scaled_dot_product_attention`
reaches the fused flash kernel, which holds no score matrix and skips the
causal triangle. That is not a preference at this shape, it is the only
path.

`shape14_harness.py --ladder`, at H=16, head_dim=64, float32, one row:

| S | scores if materialized | the call adds | ms |
|---:|---:|---:|---:|
...
Memory grows as `S`. Time grows as `S * S`. The added memory is the output
alone.

### Nothing in the model changed

`plan_kernels()` already selects `batch_chunk=1` for shape 14, and
`shape14_harness.py --coverage` shows that every other field of its plan is
also selected by a shape the graded harness runs:

    batch_chunk=1        UNIQUE TO SHAPE 14
    every other field    covered by shapes 1, 3-6 and 8-13

Only the *value* is new. Shape 6 runs the same loop (row 10) at
`batch_chunk=1024`. So this row adds a harness, not an optimization.

The harness does not call `UserOptimizedTransformer.forward()`. That
concatenates all 32 chunk outputs into one 12.2 GiB array, beside a 12.2 GiB
input, which needs over 36 GiB on an 18 GiB machine. The harness drives the
chunk loop itself, and it reduces each row before it takes the next.

### Accuracy, with no baseline to compare against

The model is causal, and LayerNorm, the FFN and the residual adds work one
token at a time. So output row `i` depends on input rows `0..i` only. Two
checks follow from that.

**Check A, `--check`.** Give the *unmodified* `BaselineTransformer` the
first 1024 tokens. That is a 64 MiB score matrix, and the CPU holds it. Its
answer must equal the first 1024 rows of the full shape 14 answer. This uses
the graded class and the graded `compare_outputs()`, both unchanged.

    row 0: max_abs 2.027e-06, max_rel 5.657e-01, 0/1048576 failed, PASS
    row 1: max_abs 2.027e-06, max_rel 1.025e+01, 0/1048576 failed, PASS

against the harness `atol = 0.002`. The `max_rel` column is large where the
reference is near zero, and the OR condition of `compare_outputs()` covers
it, exactly as in the 13-shape sweep.

**Check B, `--tail`.** Check A cannot reach row 99999, which attends every
key. `frugal_forward()` repeats the baseline arithmetic in query blocks, so
it never materializes the score matrix. `--validate` proves it equal to
`BaselineTransformer` where both can run: **48 of 48 cases PASS over 12
shapes, 2 padding settings and 2 query-block counts, and 42 are bit exact.**
The 6 that are not read `max_abs` 9.5e-07 to 1.43e-06, and all 6 are a
blocked run, so the difference is the shorter reduction.

The first version of `--validate` was worthless and said PASS anyway. Every
shape it ran had `seq_len` at most 128, against a default `q_block` of 256,
so the query loop never ran a second block and the "blocked" path was the
unblocked path. It now runs each case at a block below its own `seq_len` as
well, and it includes shape 13, which is the only case with a long sequence
and a real answer to check against.

`frugal_forward()` is a reference. It is NOT the graded baseline.

**NOT RUN YET at shape 14.** The tail check needs about 15 minutes of CPU.
Check A covers the first 1024 rows of 100000.

### The measurement

    .venv/bin/python3 shape14_harness.py --time --rows 32

at commit 311a420*, one row for each of the 32 chunks. Every row does the
same work, so one pass gives 32 samples.

| Item | Value |
|---|---|
| per row, median compute | 15.766 s (min 15.681, max 17.700) |
| full batch, compute | **504.5 s = 8.41 min** |
| full batch, with the framework copy | 505.6 s |
| framework boundary | 0.2% of the call |
| tokens/s | 6,343 |
| CPU | cannot run |
| MPS | cannot run |
| speedup | none. There is no baseline time to divide by |

The boundary is 0.2% here, against 3.4% at shape 6 (row 52). The copy is the
same 391 MiB per chunk, and the compute it hides behind is 340x longer.

### Read the MFU from the executed column
...
calculates. The fused kernel runs the triangle only, which is 51.5% of the
counted work. So the counted MFU passes 100%, and that is not an error in
the stopwatch. `references/scoreboard.md` already records this effect for
shape 13, at 73% attention. Shape 14 is 97% attention, so it is close to the
full 2x.

**The executed 67.9% of the 4.06 TFLOP/s ceiling is the honest reading.**
