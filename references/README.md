...
| [figures/](figures/) | Diagrams of the design. Read [figures/README.md](figures/README.md) before you move or delete one: no script can rebuild them |
| [wwdc/](wwdc/) | Apple GPU talk transcripts: A14 Metal, Apple silicon Metal, and Metal shader tuning |
...
| `appendix_cases.py` | The 14 shapes as code, with per-case selection. It runs 1 to 13; shape 14 stays disabled here. |
| `shape14_harness.py` | Shape 14 alone. The baseline cannot run it, so this file supplies its own accuracy checks and reports no speedup. Row 55. |
...
| `profiling/probes/ln_tiled_stats_probe.py` | The accuracy screen for row 47. It measures the residual drift, which decides whether a tiled reduction can use the uncentred variance. |
| `profiling/probes/plan_ab.py` | A/B one `KernelPlan` field in one process, interleaved. A shape under about 2 ms moves further with the machine than with the code, so a two-sweep ratio cannot score it. |
| `profiling/probes/pipeline_probe.py` | Whether a CPU copy hides behind GPU work. It does not: on unified memory the two contend for one controller, and overlapping is worse than serial. Row 48. |
